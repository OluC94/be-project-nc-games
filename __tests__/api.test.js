  ...
  test("200: responds with JSON containing appropriate details of all avaiable endpoints", () => {
    return request(app)
      ...
      .then(({ body }) => {
        ...
        expect(typeof body["GET /api/reviews/:review_id"].description).toBe(
          "string"
        );
        expect(body["GET /api/reviews/:review_id"].queries).toEqual([
          "category",
          "sort_by",
          "order",
        ]);
        expect(
          body["GET /api/reviews/:review_id"].exampleResponse.review
        ).toMatchObject({
          review_id: expect.any(Number),
          title: expect.any(String),
          category: expect.any(String),
