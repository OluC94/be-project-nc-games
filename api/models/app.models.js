...
exports.fetchReviews = (category, sort_by = "created_at", order = "desc") => {
  ...
  queryStr += `
    ...
    ORDER BY ${sort_by} ${orderStr};`;

  return db
    .query(queryStr, queryValuesArr)
    .then(({ rows }) => {
      if (rows.length === 0) {
...
exports.fetchReviewByID = (review_id) => {
  ...
  return db
    .query("SELECT review_id FROM comments")
    .then(({ rows }) => {
      let queryStr = "";
...
exports.editReview = (review_id, patchData) => {
  ...
  return db
    .query(queryStr, [patchData.inc_votes, review_id])
    .then(({ rows }) => {
      if (rows.length === 0) {
        return Promise.reject({ status: 404, msg: "Review not found" });
      } else {
...
exports.removeCommentByCommentID = (comment_id) => {
  ...
  return db.query(queryStr, queryValuesArr).then((deletedItem) => {
